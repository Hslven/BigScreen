    ...
    <div class="header" v-if="header.length && mergedConfig" :style="`background-color: ${mergedConfig.headerBGC};`">
      <div
        ...
        :style="`
          ...
          width: ${mergedConfig.columnWidth[i]}px;
        `"
        style="color: #56BFFF; font-size: 14px; font-weight: 500; letter-spacing: 2px;"
        :align="aligns[i]"
        v-html="headerItem"
      />
    </div>
    ...
    >
      ...
      >
        <div
          ...
          :key="`${ceil}${ri}${ci}`"
          :style="`width: ${mergedConfig.columnWidth[ci]}px;`"
          :align="aligns[ci]"
          @click="emitEvent('click', ri, ci, row, ceil)"
          @mouseenter="handleHover(true, ri, ci, row, ceil)"
          @mouseleave="handleHover(false)">
          <t-popup class="scroll-popup" placement="left" attach="#myPopup">
            <div class="ceil" v-html="ceil"></div>
...
export default Vue.extend({
  ...
  methods: {
    ...
    calcData () {
      ...
      const { calcHeights, calcAligns } = this
    ...
    mergeConfig () {
      ...
      this.mergedConfig = deepMerge({}, cloneDeep(defaultConfig), config || {})
    },
    calcHeaderData () {
...
/deep/ .t-popup .t-popup__content {
  background: rgba(37, 91, 163) !important;
  color: #AFBDD5 !important;
}
</style>
