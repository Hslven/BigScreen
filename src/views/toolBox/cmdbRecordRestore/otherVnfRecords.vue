  ...
  data () {
    return {
      ...
      columns: [
        ...
        { colKey: 'data.name', title: '无主VNF', align: 'left' },
        ...
        { colKey: 'data.networkElemt', title: '所属网元', align: 'left' },
      ...
      return new Promise((resolve, reject) => {
        getDataRecordInfoApi(this.recordInfo).then(response => {
          ...
          if (data.code === 0) {
            ...
            this.$message.error(`获取无主VNF档案数据失败：${data.message}`)
          }
        }).catch(error => {
          this.$message.error(`获取无主VNF档案数据失败：${error}`)
        })
      })
    },
    onDataRestore (row) {
