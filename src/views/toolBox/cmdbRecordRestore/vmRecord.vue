  ...
  <div>
    ...
    <t-table
      ...
      <template #operation="{ row }">
        <t-link class="pr-2" theme="primary" hover="color" @click="onHistory('vm', row.name)">
          历史操作记录
        </t-link>
        <t-link class="pr-2" theme="primary" hover="color" v-if="row.record" @click="otherRecord('vm', row.name)">
          其他档案
        </t-link>
      </template>
    </t-table>
    ...
    <t-dialog width="80%" :visible.sync="isOtherRecords" header="其他档案" :cancelBtn="null" :confirm-btn="null">
      <other-vm-records v-if="isOtherRecords" @refreshData="getData" :isOtherRecords.sync="isOtherRecords" :recordInfo="otherRecordInfo"></other-vm-records>
    </t-dialog>
...
import OtherVmRecords from '@/views/toolBox/cmdbRecordRestore/otherVmRecords'
import OperateHistory from '@/views/toolBox/cmdbRecordRestore/operateHistory'
import globalSearch from '@/mixins/globalSearch'
export default {
  name: 'vmRecord',
  mixins: [globalSearch],
  components: { SearchIcon, DownloadIcon, RefreshIcon, OtherVmRecords, OperateHistory },
  data () {
    return {
      resourceType: '网络云',
      resourceCategory: [
      ...
      columns: [
        ...
        { colKey: 'record.name', title: '档案名称', align: 'left', ellipsis: { theme: 'light', placement: 'bottom' } },
        { colKey: 'record.type', title: '档案类型', align: 'left' },
        { colKey: 'record.createTime', title: '档案创建时间', align: 'left', ellipsis: { theme: 'light', placement: 'bottom' } },
        { colKey: 'record.author', title: '档案创建人', align: 'left' },
        { colKey: 'record.data.mgmSys', title: '部署管理系统', align: 'left', ellipsis: { theme: 'light', placement: 'bottom' }, sortType: 'all', sorter: (a, b) => a.record.data ? 1 : b.record.data ? -1 : 0 },
        { colKey: 'record.data.remark', title: '备注', align: 'left' },
        { colKey: 'record.data.vnf', title: '所属VNF', align: 'left', sortType: 'all', sorter: (a, b) => a.record.data ? 1 : b.record.data ? -1 : 0, ellipsis: { theme: 'light', placement: 'bottom' } },
        { colKey: 'operation', align: 'left', title: '操作', width: '200' }
      ],
      pagination: {
        defaultCurrent: 1,
        defaultPageSize: 12,
  ...
  methods: {
    ...
    getData () {
      ...
      return new Promise((resolve, reject) => {
        getAbnormalDataApi({ type: 'vm', resource_type: this.resourceType }).then(response => {
          const { data } = response
          if (data.code === 0) {
            this.data = data.data
            ...
            this.$message.error(`获取无主虚拟机数据失败：${data.message}`)
          }
        }).catch(error => {
          this.$message.error(`获取无主虚拟机数据失败：${error}`)
        })
      })
    },
    ...
    onDataRestore () {
      ...
      return new Promise((resolve, reject) => {
        restoreDataApi(restoreFrom).then(response => {
          const { data } = response
          if (data.code === 0) {
            this.$message.success('vm类型同步档案成功!')
            this.getData()
          } else {
            this.$message.error(`vm类型档案同步失败：${data.message}`)
          }
        }).catch(error => {
          this.$message.error(`vm类型档案同步失败：${error.data.message}`)
        })
      })
    },
    downloadFile () {
      ...
      exportJsonToExcel(header, jsonData, '虚拟机名称')
    },
