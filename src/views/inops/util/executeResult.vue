<template>
  <div>
    ...
    <div class="bg-mod-card" style="margin-top: 15px; padding: 15px">
      <t-row>
        <t-col :md="8" :sm="6">
          <div>
            <span style="margin-right: 10px"><span style="font-size: 20px; font-weight: 700; color: #165DFF; margin-right: 10px">{{ transProgress }}</span>通过</span>
          ...
          <div style="margin-top: 10px">
            <t-progress theme="plump" :percentage="progress * 100" :label="false" />
          </div>
        </t-col>
        <t-col :offset="1" :md="3" :sm="5">
          <t-row style="padding-top: 2px">
            <t-col :span="4" align="center" v-for="(item, index) in countInfo" :key="index">
              <div style="font-size: 24px; font-weight: 700;" :style="{ color: item.color }">{{ item.sum}}</div>
    ...
    <div class="bg-mod-card" style="margin-top: 20px; padding: 20px 16px">
            ...
            >
              <t-button variant="outline" theme="default">
                <t-row align="center">
                  ...
                  <span style="color: rgba(80, 89, 104, 1)">全量数据</span>
                </t-row>
              </t-button>
            </t-dropdown>
          </t-row>
      ...
      <t-row style="margin-top: 15px">
        <t-col :span="6" style="padding-right: 10px">
          <div style="height: 460px">
            ...
            >
              ...
              <template #operator="{ row }">
                <t-button theme="primary" variant="text" @click="downLoad('single', row.id)">下载</t-button>
              </template>
            </t-table>
          </div>
        ...
        <t-col :span="6" style="padding-left: 10px">
          <div id="scroll" style="background-color: #000000; padding: 15px; height: 500px; overflow-y: auto; white-space: pre-wrap">
...
export default {
    ...
    return {
      ...
      table: {
        ...
        column: [
          ...
          { colKey: 'operator', title: '操作', align: 'center', resizable: true, ellipsis: true, ellipsisTitle: true }
        ]
      },
      pages: {
        list: pageList,
  ...
  mounted () {
    this.paramInfo = JSON.parse((JSON.stringify(this.$route.query.obj)))
    this.backUrl = this.paramInfo.backUrl
    this.setData()
    this.isNewData()
  },
  deactivated () {
  ...
  methods: {
    ...
    isNewData () {
      if (this.paramInfo.isNew) {
        this.initData()
        this.initws(this.paramInfo.taskId)
      }
    },
    setData () {
      const { name, type } = this.paramInfo
      this.info.name = name
      this.info.type = this.dictMapper[type]
    },
    initData () {
      ...
      this.table.infoData = []
      this.table.data = []
      this.info = Object.assign(this.info, { jobId: '', startTime: '-', consuming: '-' })
      this.isFinish = false
