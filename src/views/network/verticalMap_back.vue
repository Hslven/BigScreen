...
import { getVerticalMapApi, downloadVerticalMapApi } from '@/api/network/network'
  ...
  methods: {
    getData (fromData) {
      return new Promise((resolve, reject) => {
        getVerticalMapApi(fromData).then(response => {
          const { data } = response
          if (data.code === 0) {
              ...
              return (
                <div>
                  <p style="white-space:pre-line">
                    {row[item.colKey] ? row[item.colKey].replaceAll(';', '\n') : row[item.colKey]}
                  </p>
                </div>
              )
            }
    ...
    exportEvent () {
      return new Promise((resolve, reject) => {
        ...
        downloadVerticalMapApi(downloadVisibleForm).then(response => {
